...
There are four mechanisms that can trigger a checkpoint, and they are mutually independent: \texttt{IO::checkpoint\_ID} (checkpoint the initial data), \texttt{IO::checkpoint\_every} (every so many iterations), \texttt{IO::checkpoint\_every\_walltime\_hours} (after so much wall time has passed), and \texttt{IO::checkpoint\_on\_terminate} (checkpoint before terminating). Each mechanism examines only its own state, and in particular a checkpoint produced by one of them never postpones another. Independently of how many mechanisms fire, \CarpetX{} writes at most one checkpoint per iteration -- a second checkpoint of the same iteration would hold the same data.
