...
namespace MovingBoxToy {

template <typename T> constexpr auto pow2(T val) noexcept -> T {
  return val * val;
}
...
extern "C" void MovingBoxToy_MoveBoxes(CCTK_ARGUMENTS) {
  ...
  // Radius of each box
  const auto R1 =
      sqrt(pow2(position_x[0]) + pow2(position_y[0]) + pow2(position_z[0]));
  const auto R2 =
      sqrt(pow2(position_x[1]) + pow2(position_y[1]) + pow2(position_z[1]));
