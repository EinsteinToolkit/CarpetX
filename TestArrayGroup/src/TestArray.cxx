  ...
  for (int n = 0; n < nmax; n++) {
    for (int j = 0; j < jmax; j++) {
      for (int i = 0; i < imax; i++) {
        ...
        test_complex1[index] = CCTK_COMPLEX(3 + (i + 1) * (j + 1) * (n + 1)) + 1i;
        test_complex2[index] = CCTK_COMPLEX(3 + 7 * (i + 1) * (j + 1) * (n + 1)) + 1i;
        test_complex3[index] = CCTK_COMPLEX(3 + 13 * (i + 1) * (j + 1) * (n + 1)) + 1i;
      }
    }
  }
...
extern "C" void TestArrayGroup_Compare(CCTK_ARGUMENTS) {
  ...
  for (char const * const array : arrays) {
    for (unsigned int suffix = 0 ; suffix < 3 ; ++suffix) {
      ...
      for (int n = 0; n < nmax; n++) {
        ...
        is_contiguous = ((char*)varptr - (char*)varptr0)/(CCTK_VarTypeSize(vartype) * size) == n;
        for (int j = 0; j < jmax; j++) {
          for (int i = 0; i < imax; i++) {
            ...
            bool is_equal = [&](){
              switch(vartype) {
                case CCTK_VARIABLE_REAL:
                  return static_cast<CCTK_REAL*>(varptr)[index] == CCTK_REAL(1 + ival);
                case CCTK_VARIABLE_INT:
                  return static_cast<CCTK_INT*>(varptr)[index] == CCTK_INT(2+ ival);
                case CCTK_VARIABLE_COMPLEX:
                  return static_cast<CCTK_COMPLEX*>(varptr)[index] == CCTK_COMPLEX(3 + ival) + 1i;
                default:
        ...
        if (array_error_count > 0) {
          CCTK_VWARN(CCTK_WARN_ALERT, "TestArrayGroup: grid array %s failed in %d of %d elements",
                      vname.str().c_str(), array_error_count, size);
          found_error = true;
        }
        if (!is_contiguous) {
          CCTK_VWARN(CCTK_WARN_ALERT, "TestArrayGroup: grid array %s is not contiguous in the grid array vector",
                      vname.str().c_str());
          found_error = true;
        }
      }
  ...
  if (gf_error_count > 0) {
    ...
    CCTK_VWARN(CCTK_WARN_ALERT, "TestArrayGroup: grid function test failed in %d of %d elements",
                gf_error_count, size);
   found_error = true;
  }
  ...
  if(*test_scalar_complex != CCTK_COMPLEX(3)) {
    CCTK_VWARN(CCTK_WARN_ALERT, "TestArrayGroup: CCTK_COMPLEX grid scalar failed");
   found_error = true;
  }
}

extern "C" void TestArrayGroup_Terminate(CCTK_ARGUMENTS) {
  // this should be caugt by output files, but since we test the array code itself, report an error explicitly
  if(found_error)
